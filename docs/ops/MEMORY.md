...
- 2026-08-20 — 2026-08-20 TASK-008: the channel is a board, not a feed. 13 hourly slots 09:00-21:00 Tehran carry BOTH report types; the first run in a slot posts, the five after it rewrite via editMessageText. reports.telegram_message_id on the delivered row IS the message id - an hourly panel and its report row have the same lifetime, so EXTENSIONS Y's panel registry table was NOT built and must not be. The report row is the message, not an archive: an edit moves content, snapshot_id and sent_at
- 2026-08-20 — 2026-08-20 slot_tolerance_minutes renamed slot_window_minutes and made ONE-SIDED (backward only): a run belongs to the last slot it has passed, never one ahead. A scheduler is late, never early. The symmetric window lost the 17:00 slot on 2026-08-16 outright - nearest runs were 22.7 min early and 20.4 min late against +/-20. Bound is now the gap between slots (60), not the collection interval
